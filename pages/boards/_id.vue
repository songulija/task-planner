<template>
  ...
  >
    ...
    <!-- looping through list of board lists. showing list titles, and delete icon -->
       <!-- making these list dragable. allowing to drop. detects drop event and dragover event.
        then  passing event values, and list.id-->
       <div
      ...
      <!-- dialog box that apeas on create -->
            ...
            <v-container>
              <v-row>
                <v-col cols="12">
                  ...
                  <v-row>
                  <v-col cols="6">
                    <p class="subtitle-1">Start date</p>
                    <v-text-field v-model="card.startDate" outlined dense hide-details>
                        <template v-slot:append-outer>
                        <date-picker v-model="card.startDate" />
                        </template>
                    </v-text-field>
                  </v-col>
                  <v-col cols="6">
                    <p class="subtitle-1">Finish date</p>
                    <v-text-field v-model="card.endDate" outlined dense hide-details>
                        <template v-slot:append-outer>
                        <date-picker v-model="card.endDate" />
                        </template>
                    </v-text-field>
                  </v-col>
                  </v-row>
              ...
              <v-row>
                <v-col cols="12">
                  ...
                  ></v-text-field>
                   ...
                   <v-row>
                  <v-col cols="6">
                    <p class="subtitle-1">Edit Start date</p>
                    <v-text-field v-model="currentCard.startDate" outlined dense hide-details>
                        <template v-slot:append-outer>
                        <date-picker v-model="currentCard.startDate" />
                        </template>
                    </v-text-field>
                  </v-col>
                  <v-col cols="6">
                    <p class="subtitle-1">Edit Finish date</p>
                    <v-text-field v-model="currentCard.endDate" outlined dense hide-details>
                        <template v-slot:append-outer>
                        <date-picker v-model="currentCard.endDate" />
                        </template>
                    </v-text-field>
                  </v-col>
                  </v-row>
...
<script>
// it will asume that dynamic id will be provided here.
  ...
  methods: {
    async createList() {
      ...
      if (that.list.title != '') {
        ...
        that.list.id = uuidv4()
        // for list adding empty cards array. that will store array of cards
        that.list.cards = []
        that.list.dateCreated = Date.now()
        // adding new list to "board" object "lists" array. and save
        if (that.board.lists) {
    ...
    drop(ev, listId) {
        // when droping card to another list, listId passes here
        // then updateCard with new list provided
      ev.preventDefault()
    ...
    async createCard(){
      ...
      if (that.card.title != '') {
        ...
        if (that.board.lists) {
          let index = -1
          let count = 0
        //   pass through lists to know index of list that i need to add "card" to
          for (const list of that.board.lists) {
